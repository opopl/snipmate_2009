...
" snippets_dir - comma-separated list of snippets directories

if !exists('g:snippets_dir')
	let g:snippets_dir = ''
endif
let d = split(substitute(globpath(&rtp, 'snippets/'), "\n", ',', 'g'),',')
call extend(d,split(g:snippets_dir,','))
let g:snippets_dir = join(d,',')
...
fun! ResetSnippets(ft)
	let ft = a:ft == '' ? '_' : a:ft
	for dict in [g:snippets, s:multi_snips, g:did_ft]
		if has_key(dict, ft)
			unlet dict[ft]
		endif
	endfor
...
fun! ResetAllSnippets()
	let g:snippets = {} | let s:multi_snips = {} | let g:did_ft = {}
endf
...
fun! GetSnippets(dir, filetypes)
	...
	for ft in split(a:filetypes, '\.')
			...
			call s:DefineSnips(dir, ft, ft)
			if ft == 'objc' || ft == 'cpp' || ft == 'cs'
				call s:DefineSnips(dir, 'c', ft)
			elseif ft == 'xhtml'
				call s:DefineSnips(dir, 'html', 'xhtml')
			endif
			let g:did_ft[ft] = 1
...
fun! MakeSnip(scope, trigger, content, ...)
	...
	let var = multisnip ? 's:multi_snips' : 'g:snippets'
...
fun s:GetSnippet(word, scope)
	...
	while snippet == ''
		if exists('g:snippets["'.a:scope.'"]["'.escape(word, '\"').'"]')
			let snippet = g:snippets[a:scope][word]
...
fun! ShowAvailableSnips()
	...
	let matches = []
	for scope in [bufnr('%')] + split(&ft, '\.') + ['_']
		let triggers = has_key(g:snippets, scope) ? keys(g:snippets[scope]) : []
